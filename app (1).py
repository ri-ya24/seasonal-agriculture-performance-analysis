...
.hero h1 {color:#0f172a !important;margin-bottom:4px}
...
st.sidebar.divider()
season = st.sidebar.multiselect("Season", sorted(df.Season.unique()), default=sorted(df.Season.unique()))
state = st.sidebar.multiselect("State", sorted(df.State.unique()), default=sorted(df.State.unique()))
crop = st.sidebar.multiselect("Crop", sorted(df.Crop.unique()), default=sorted(df.Crop.unique()))
irrigation = st.sidebar.multiselect("Irrigation Method", sorted(df.Irrigation_Method.unique()), default=sorted(df.Irrigation_Method.unique()))

f = df[
    df["Season"].isin(season) &
    df["State"].isin(state) &
    df["Crop"].isin(crop) &
    df["Irrigation_Method"].isin(irrigation)
].copy()
...
def chart(fig):
    fig.update_layout(
        ...
        title=dict(font=dict(color="#0f172a", size=18), x=0.02),
        legend=dict(font=dict(color="#334155"))
    )
    fig.update_yaxes(gridcolor="#e2e8f0", tickfont=dict(color="#334155"), title_font=dict(color="#334155"))
    fig.update_xaxes(showgrid=False, tickfont=dict(color="#334155"), title_font=dict(color="#334155"))
